...
4. Return to main FixFlow folder and verify your docker-compose.yaml.  Recommended is below:
```YAML
services:
  nginx-proxy:
    image: nginx:latest
...
5. Run the following commands from the FixFlow folder:
````Bash
docker compose build --no-cache
docker compose up -d
````

6. During the stacks first run a minimum configuration is seeded into the DB. This includes the default admin credentials.
   These initial credentials will only work once and can be obtained from the docker logs for the fixflow.web container, scroll the log to find an entry similar to this:
````Text
					==================================================
...
7. Navigate to ASSIGNED-IP:80 and use the default admin credentials. You will be forced to change upon first use.
